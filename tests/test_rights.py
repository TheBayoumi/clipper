...
def test_explicit_video_allowlist_is_authoritative() -> None:
    assert_video_allowed(brief(), video())
    assert_video_allowed(brief(allowed_video_ids=["v2"]), video("v2", "other"))


def test_rights_confirmation_and_explicit_video_allowlist_are_enforced() -> None:
    with pytest.raises(RightsError, match="rights_confirmed"):
        assert_campaign_authorized(brief(rights_confirmed=False))
    with pytest.raises(RightsError, match="explicit target"):
        assert_video_allowed(brief(source_channel_ids=["UC1"]), video("v2", channel_id="UC1"))
