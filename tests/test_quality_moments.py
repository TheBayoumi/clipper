...
def _graph(core_id: str, offset: int = 0) -> tuple[SemanticCore, NarrativeEnvelope]:
    words = tuple(
        CanonicalWord(
...
def _window(core: SemanticCore, envelope: NarrativeEnvelope, suffix: str, score_offset: float = 0.0) -> FeasibleDeliveryWindow:
    del score_offset
    return FeasibleDeliveryWindow(
        ...
        envelope.source_start + 20.0,
        envelope.source_word_ids[:20],
    )
...
def test_quality_yield_equals_number_of_independent_passing_cores(count: int) -> None:
    ...
    cores = tuple(item[0] for item in graph)
    envelopes = tuple(item[1] for item in graph)
    windows = tuple(_window(core, envelope, "a") for core, envelope in graph)
    assessments = tuple(
...
def test_multiple_passing_windows_for_one_core_still_produce_one_quality_moment() -> None:
    core, envelope = _graph("core")
    first = _window(core, envelope, "first")
    second = _window(core, envelope, "second")
...
def test_rejected_or_escalated_windows_do_not_become_quality_moments() -> None:
    core, envelope = _graph("core")
    window = _window(core, envelope, "a")
