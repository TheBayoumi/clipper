...
from . import mw4_semantic_gameplay_v3 as semantic
...
def _append_segment(
    ...
    parts.append(f"[0:v]trim=start={start:.3f}:end={end:.3f},setpts={video_setpts}[{video_label}]")
    audio_chain = f"[0:a]atrim=start={start:.3f}:end={end:.3f},asetpts=PTS-STARTPTS"
...
def _effect_times(plan: semantic.SemanticPlan, teaser_seconds: float) -> list[float]:
    ...
    for event in plan.effect_events:
        output_time = semantic.source_time_to_output(
            event.time, plan.segments, prefix_seconds=prefix
        )
    ...
    if profile == "precision_punch" and times:
        ...
        parts.extend(
            [
                ...
                f"[base][z1]overlay=0:0:enable='{gate([first], 0.08, 0.23)}'[fx1]",
                f"[fx1][z2]overlay=0:0:enable='{gate([second], 0.10, 0.30)}'[outv]",
            ]
        )
        ...
        parts.extend(
            [
                ...
                f"[fx1]drawbox=x=0:y=0:w=iw:h=ih:color=white@0.10:t=fill:enable='{gate(impact_times, 0.01, 0.05)}'[outv]",
            ]
        )
        ...
        parts.extend(
            [
                ...
                f"[base][z]overlay=0:0:enable='{gate([first, later], 0.08, 0.26)}'[outv]",
            ]
        )
        ...
        parts.extend(
            [
                ...
                f"[base][z]overlay=0:0:enable='{gate([strongest], 0.06, 0.18)}'[outv]",
            ]
        )
    else:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="MW4 V3 semantic gameplay editor + deterministic 250 Mbps renderer"
    )
