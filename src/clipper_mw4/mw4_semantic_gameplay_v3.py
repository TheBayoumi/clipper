...
def _protect_range(
    start: float, end: float, events: list[SemanticEvent], radius: float
) -> tuple[float, float] | None:
    left, right = start, end
    for event in events:
        if (
            event.kind not in {"combat_burst", "outcome_like", "impact", "contact"}
            or event.confidence < 0.55
        ):
            continue
...
def _editorial_segments(
    ...
    protection = float(cfg.get("event_protection_seconds", 0.38))
    max_actions = int(cfg.get("max_editorial_actions", 2))
    speed = min(float(cfg.get("max_speed", 2.0)), float(cfg.get("traversal_speed", 1.75)))

    dull_ranges = _ranges(timeline.signals["dull"] > 0.5, i0, i1, fps)
    traversal_mask = (
        (timeline.signals["traversal"] >= float(cfg.get("traversal_threshold", 0.58)))
        & (timeline.signals["combat"] < 0.42)
        & (timeline.signals["outcome"] < 0.38)
        & (timeline.signals["impact"] < 0.42)
    )
    candidates: list[tuple[float, float, str, float]] = []
    for raw_start, raw_end, kind in [(a, b, "cut_dull") for a, b in dull_ranges] + [
        (a, b, "speed_traversal") for a, b in _ranges(traversal_mask, i0, i1, fps)
    ]:
    ...
    for left, right in itertools.pairwise(boundaries):
        ...
        if action is None:
            ...
            segments.append(
                EditSegment(
                    round(left, 3), round(right, 3), round(speed, 3), "compressed_traversal"
                )
            )
    ...
    for a, b in dull_ranges:
        ...
        if b <= a:
            continue
        remaining = sum(
            (min(b, s.end) - max(a, s.start)) / s.speed
            for s in segments
...
def source_time_to_output(
    source_time: float, segments: tuple[EditSegment, ...], prefix_seconds: float = 0.0
) -> float | None:
    out = prefix_seconds
...
def _route_story(
    ...
    opening: float,
    config: dict[str, Any],
) -> tuple[str, str, float | None, tuple[SemanticEvent, ...], tuple[str, ...]]:
    cfg = config.get("semantic_editor", {})
    outcomes = [
        event for event in events if event.kind == "outcome_like" and event.confidence >= 0.55
    ]
    impacts = [event for event in events if event.kind == "impact" and event.confidence >= 0.62]
    combats = [
        event for event in events if event.kind == "combat_burst" and event.confidence >= 0.55
    ]
    ...
    if strongest is not None:
        ...
        if (
            ...
            and strongest.time <= end - 0.35
            and gain >= float(cfg.get("cold_open_min_interest_gain", 0.18))
            and strongest.confidence >= 0.70
        ):
            teaser_start = min(max(start, strongest.time - 0.42), end - teaser_seconds)
            return (
                "cold_open_payoff",
                "cold_open_teaser",
                round(teaser_start, 3),
                (strongest,),
                ("later semantic payoff materially beats opening",),
            )
        ...
        for event in chain_source[index + 1 :]:
            ...
            if len(chain) == 3:
                return (
                    ...
                    None,
                    tuple(chain),
                    ("three coherent combat/outcome events",),
                )

    if impacts:
        strongest_impact = max(impacts, key=lambda event: event.confidence)
        if strongest_impact.confidence >= float(cfg.get("impact_flash_min_confidence", 0.76)):
            return (
                "explosive_impact",
                "impact_flash",
                None,
                (strongest_impact,),
                ("high-confidence audiovisual impact",),
            )
    if outcomes:
    ...
    return (
        ...
        None,
        selected,
        ("sustained pressure kept readable",),
    )


def plan_window(
    timeline: SemanticTimeline, start: float, end: float, config: dict[str, Any]
) -> SemanticPlan | None:
    cfg = config.get("semantic_editor", {})
    fps = timeline.fps
    i0, i1 = (
    ...
    combat, outcome, impact = (
        timeline.signals["combat"][i0:i1],
        timeline.signals["outcome"][i0:i1],
        timeline.signals["impact"][i0:i1],
    )
    events = _window_events(timeline, start, end)
    meaningful = [
        event
        for event in events
        if event.kind in {"combat_burst", "outcome_like", "impact"} and event.confidence >= 0.55
    ]
    ...
    low_threshold = float(cfg.get("low_interest_threshold", 0.30))
    ...
    story, profile, cold_open, effect_events, route_reasons = _route_story(
        timeline, start, end, events, opening, config
    )
    planned_duration = base_duration + (
        float(config.get("editorial", {}).get("cold_open_teaser_seconds", 1.25))
    ...
    engagement = float(
        np.clip(
            0.28 * float(np.percentile(interest, 90))
            + 0.22 * opening
            + 0.20 * event_conf
            + 0.18 * min(1, len(meaningful) / 4)
            + 0.12 * max(float(np.max(outcome)), float(np.max(impact)), float(np.max(combat))),
            0,
            1,
        )
    )
    if (
        retention < float(config.get("performance_targets", {}).get("retention_proxy_min", 0.30))
        or engagement
        < float(config.get("performance_targets", {}).get("engagement_proxy_min", 0.30))
        or weakest < float(cfg.get("minimum_weak_quarter_interest", 0.28))
        or low_fraction > float(cfg.get("maximum_low_interest_fraction", 0.42))
    ):
        return None
    ...
    return SemanticPlan(
        ...
        round(end, 3),
        round(end - start, 3),
        round(planned_duration, 3),
        round(score, 6),
        round(retention, 6),
        round(engagement, 6),
        round(mean_interest, 6),
        round(opening, 6),
        round(closing, 6),
        round(weakest, 6),
        round(low_fraction, 6),
        round(residual, 3),
        story,
        profile,
        cold_open,
...
def run_self_test() -> None:
    ...
    times = (np.arange(length, dtype=np.float32) + 0.5) / fps
    interest = np.full(length, 0.48, dtype=np.float32)
    combat = np.full(length, 0.35, dtype=np.float32)
    outcome = np.zeros(length, dtype=np.float32)
    impact = np.zeros(length, dtype=np.float32)
    contact = np.full(length, 0.34, dtype=np.float32)
    traversal = np.zeros(length, dtype=np.float32)
    recovery = np.zeros(length, dtype=np.float32)
    dull = np.zeros(length, dtype=np.float32)
    dull[int(5 * fps) : int(5.9 * fps)] = 1
    interest[int(5 * fps) : int(5.9 * fps)] = 0.16
    combat[int(5 * fps) : int(5.9 * fps)] = 0.10
    events: list[SemanticEvent] = []
    for when in (1.1, 3.4, 8.2):
        ...
        interest[index] = 0.92
        events += [
            SemanticEvent(when, "outcome_like", 0.82, {"outcome": 0.82}),
            SemanticEvent(when + 0.05, "combat_burst", 0.90, {"combat": 0.90}),
        ]
    timeline = SemanticTimeline(
        fps,
        duration,
    ...
    config = {
        ...
        "performance_targets": {"retention_proxy_min": 0.20, "engagement_proxy_min": 0.20},
        "semantic_editor": {
            "minimum_meaningful_events": 2,
            "minimum_output_seconds": 10,
            "maximum_output_seconds": 20,
            "min_dull_action_seconds": 0.65,
            "max_single_dull_action_seconds": 2.2,
            "event_protection_seconds": 0.3,
            "max_editorial_actions": 2,
            "traversal_speed": 1.75,
            "max_speed": 2,
            "max_total_removed_equivalent_seconds": 2.5,
            "max_unexplained_low_interest_run_seconds": 1,
            "minimum_weak_quarter_interest": 0.12,
            "maximum_low_interest_fraction": 0.5,
            "low_interest_threshold": 0.30,
            "cold_open_min_event_delay_seconds": 1.35,
            "cold_open_min_interest_gain": 0.18,
            "impact_flash_min_confidence": 0.76,
        },
    }
    plan = plan_window(timeline, 0, duration, config)
    if (
        plan is None
        or plan.max_unexplained_low_interest_run_seconds > 1
        or source_time_to_output(8.2, plan.segments) is None
    ):
        raise AssertionError("semantic editor self-test failed")
    print(
        json.dumps({"self_test": "PASS", "story": plan.story_type, "profile": plan.effect_profile})
    )
