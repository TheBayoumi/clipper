...
def finishing_open_plans(
    ...
    for span in timeline.finishing_moves:
        ...
        body_minimum = analysis._required_body_duration(hero, final_minimum)
        ...
        for component in components:
            ...
            for start, end in _finishing_variants(
                ...
                component_failures = analysis._component_metric_failures(
                    metrics,
                    config,
                    require_ending=False,
                )
                terminal_failures = analysis._component_metric_failures(
                    metrics,
                    config,
                    require_ending=True,
                )
        ...
        for count in range(1, max(1, max_islands) + 1):
            for group in itertools.combinations(moments, count):
                ...
                structure_failures = analysis._group_structure_failures(
                    engagements,
                    float(hero.end),
                    config,
                )
...
def self_test() -> None:
    ...
    if abs(float(analysis._required_body_duration(hero, 10.0)) - 7.57) > _EPS:
