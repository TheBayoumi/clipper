...
def _idx(timeline: SemanticTimeline | base.SemanticTimeline, seconds: float) -> int:
    fps = timeline.fps
    return max(0, min(len(timeline.times) - 1, round(seconds * fps - 0.5)))


def _mean(signal: np.ndarray, timeline: SemanticTimeline, start: float, end: float) -> float:
...
def _max(signal: np.ndarray, timeline: SemanticTimeline, start: float, end: float) -> float:
    i0 = max(0, math.floor(start * timeline.fps))
    i1 = min(len(signal), math.ceil(end * timeline.fps))
    if i1 <= i0:
        return 0.0
