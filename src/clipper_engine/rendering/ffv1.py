...
def _stage_plan_source(
    original_source: Path,
    plan: source.semantic.SemanticPlan,
    workspace: Path,
    config: dict[str, Any],
) -> tuple[Path, source.semantic.SemanticPlan, dict[str, Any], list[dict[str, Any]]]:
    ...
    pieces: list[Path] = []
    mappings: list[tuple[source.semantic.EditSegment, source.semantic.EditSegment]] = []
    ...
    for index, segment in enumerate(segments):
        ...
        piece_duration = float(source.renderer.probe(piece)["format"]["duration"])
        ...
        local_segment = source.semantic.EditSegment(
    ...
    for event in plan.effect_events:
        mapped = source._map_source_time(event.time, mappings)
        if mapped is not None:
            local_events.append(replace(event, time=round(mapped, 6)))

    local_finishing = None
    if plan.finishing_move is not None:
        start = source._map_source_time(plan.finishing_move.start, mappings)
        payoff = source._map_source_time(plan.finishing_move.payoff, mappings)
        end = source._map_source_time(plan.finishing_move.end, mappings)
...
def _render_canonical_lossless_master(
    ...
    maximum_seconds = float(config["semantic_editor"].get("maximum_output_seconds", 12.0))
    maximum_frames = int(maximum_seconds * float(timing.nominal_rate))
    if maximum_frames <= 0:
        raise RuntimeError("maximum output duration produces no legal video frames")
    frame_safe_maximum = maximum_frames / float(timing.nominal_rate)
    duration = min(float(duration), frame_safe_maximum)
    ...
    return {
        ...
        "canonical_fps": fps,
        "frame_safe_maximum_duration": round(frame_safe_maximum, 9),
        "maximum_delivery_frames": maximum_frames,
